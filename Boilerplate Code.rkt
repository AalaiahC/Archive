...
(define (expval-extractor-error variant value)
  (eopl:error 'expval-extractors "Looking for a ~s, given ~s" variant value))
...
(define (value-of exp env)
  (cases expression exp
    ...
    (letrec-exp (p-names params p-bodys letrec-body)
                (let* ((proc-names (map proc-val p-names))
                       (proc-bodies (map (lambda (b) (procedure params b env)) p-bodys))
                       (rec-env (foldr (lambda (name proc body env)
                                         (extend-env name proc (extend-env name body env)))
                                       env
                                       proc-names
                                       proc-bodies)))
...
• proc-val: (listof symbol) expression env → expval

Observers
• expval->int : expval → int
...
• Specifying the behavior of programs
     Constructor a-program: expression → program
     Observer (value-of-program e) = (value-of e ρinit)

• Specifying the behavior of expressions
     Constructors const-exp, true-exp, false-exp, diff-exp, etc.
     Observer value-of: expression env → expval
|#
