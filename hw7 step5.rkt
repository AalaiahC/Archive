...
(define value-of-cps
  (lambda (expr env-cps k)
    (match expr
      ...
      [`(mult ,nexp1 ,nexp2) (value-of-cps nexp1 env-cps (lambda (v)
                                                           (value-of-cps nexp2 env-cps (lambda (w)
                                                                                         (apply-k k (* v w))))))]
      [`(sub1 ,nexp) (value-of-cps nexp env-cps (lambda (v)
                                                  (apply-k k (sub1 v))))]
      [`(zero ,nexp) (value-of-cps nexp env-cps (lambda (v)
                                                  (apply-k k (zero? v))))]
      [`(if ,test ,conseq ,alt) (value-of-cps test env-cps (lambda (b)
                                                             (if b
                                                                 (value-of-cps conseq env-cps k)
                                                                 (value-of-cps alt env-cps k))))]
      [`(letcc ,body) (value-of-cps body (extend-env k env-cps) k)]
      [`(throw ,k-exp ,v-exp) (value-of-cps k-exp env-cps (lambda (k)
                                                            (value-of-cps v-exp env-cps k)))]
